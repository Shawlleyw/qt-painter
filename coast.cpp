    ...
    ui(new Ui::Coast)
{
    ...
    output_window.resize(800, 450);
    output_window.move(400, 500);
}
...
bool Coast::check(QPoint cur){
    int x = cur.rx(), y = cur.ry();
    if(x >= dx1 && x <= 800 + dx1 && y >= dy1 && y <= dy1 + 450)
        return true;
    return false;
}
